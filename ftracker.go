...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	averageSpeedInMeters := meanSpeed(action, duration) * kmhInMsec
	heightInMeters := height / cmInM
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(averageSpeedInMeters, 2)/heightInMeters)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
